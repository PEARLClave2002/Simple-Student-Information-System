...
def my_treeview():
    ...
    style.configure("Treeview", background = "gold", foreground = "black", fieldbackground = "gold")
...
def update_student():
    ...
    with open(ssis_csv, 'r') as readFile:
        ...
        for row in reader:
            lines.append(row)
            for field in row:
                if field == studentData:
                    lines.remove(row)
                    lines.insert(count, [idnumber_entry.get(), name_entry.get(),course_entry.get(),yearlevel_entry.get(), gender_entry.get()])
            count += 1
    with open(ssis_csv, "w", newline = "") as writeFile:
...
def select_record():
    ...
    yearlevel_entry.insert(0, values[3])
...
def add_student_widget():
    ...
    #buttons
    ConfirmAdd = Button(SSIS, text = "Add Student",
                     font = ("Arial Black", 12),
                     fg = "white",
                     bg = "blue",
                     bd = 4,
    ...
    ConfirmUpdate = Button(SSIS, text = "Confirm Update",
                     font = ("Arial Black", 12),
                     fg = "white",
                     bg = "blue",
                     bd = 4,
    ...
    ConfirmAdd.place(x = 445, y = 575)
    ConfirmUpdate.place(x = 1038, y = 575)
